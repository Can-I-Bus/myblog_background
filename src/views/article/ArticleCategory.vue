<template>
    <div class="article_category_wrap">
        <el-button type="primary" style="margin-bottom: 10px" @click="showCategoryAction = true"> 添加父级分类 </el-button>
        <el-table :data="categoryList" class="table" border empty-text="暂无数据" row-key="id">
            <el-table-column label="ID" prop="id" align="center" />
            <el-table-column label="图标" align="center">
                <template #default="scope">
                    <img style="width: 50px; height: auto" :src="scope.row.icon" />
                </template>
            </el-table-column>
            <el-table-column label="博客类别" prop="name" align="center" />
            <el-table-column label="文章数量" prop="article_count" align="center" />
            <el-table-column label="操作" prop="id" align="center">
                <template #default="scope">
                    <el-button @click="handleEdit(scope.row)" type="primary" size="small">编辑</el-button>
                    <el-button @click="handleDelete(scope.row.id)" type="danger" size="small">删除</el-button>
                    <el-button @click="handleAddChild(scope.row.id)" type="success" size="small">添加子级分类</el-button>
                </template>
            </el-table-column>
        </el-table>
        <ArticleCategoryAction ref="categoryAction" :isShow="showCategoryAction" @close="showCategoryAction = false" @update="getCategoryList" />
    </div>
</template>
<script setup>
...
const handleEdit = async (row) => {
    ...
    categoryAction.value.setIcon(row.icon);
