<template>
    <div>
        <el-dialog :title="editData.id === '' ? '添加Banner' : '编辑Banner'" width="" :model-value="props.isShow" :before-close="handleClose">
            <el-form ref="formRef" :model="editData" :rules="rules" label-width="110px">
                <el-form-item label="标题" prop="title">
                    <el-input v-model="editData.title"></el-input>
                </el-form-item>
                <el-form-item label="描述" prop="description">
                    <el-input rows="4" v-model="editData.description" type="textarea"></el-input>
                </el-form-item>
                <el-form-item label="banner展位图" prop="mid_img">
                    <div class="upload_box">
                        <Upload
                            ref="uploadRef"
                            @fileHandler="
                                (file) => {
                                    fileHandler(file, 'mid_img');
                                }
                            " />
                    </div>
                </el-form-item>
                <el-form-item label="banner" prop="big_img">
                    <div class="upload_box">
                        <Upload
                            ref="uploadRef2"
                            @fileHandler="
                                (file) => {
                                    fileHandler(file, 'big_img');
                                }
                            " />
                    </div>
                </el-form-item>
...
const dataSet = (data) => {
    ...
    setTimeout(() => {
        ...
        uploadRef2.value.setUrl(data.big_img);
    }, 100);
};

const fileHandler = async (file, param) => {
    ...
    if (res.code === 0) {
        ...
        editData.value[param] = res.data.file.url;
    } else {
...
const getData = () => {
    const data = {
        ...
        big_img: editData.value.big_img,
    };
